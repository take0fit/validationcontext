...
func init() {
	...
	voauto.Register("user_valueobject_profile_NewAge",
		func(v any, vc *validationcontext.ValidationContext) any {
			// Handle different integer types
			intVal, err := convertToInt(v)
			if err != nil {
				vc.AddError("user_valueobject_profile_NewAge", fmt.Sprintf("Type conversion error: %v", err))
				return NewAge(0, vc) // Return zero value on error
			}
			return NewAge(intVal, vc)
		})
	// Fully qualified key (unique across different packages)
	voauto.Register("user_valueobject_profile_NewFirstName",
		func(v any, vc *validationcontext.ValidationContext) any {
			...
			if err != nil {
				vc.AddError("user_valueobject_profile_NewFirstName", fmt.Sprintf("Type conversion error: %v", err))
	...
	voauto.Register("user_valueobject_profile_NewLastName",
		func(v any, vc *validationcontext.ValidationContext) any {
			...
			strVal, err := convertToString(v)
			if err != nil {
				vc.AddError("user_valueobject_profile_NewLastName", fmt.Sprintf("Type conversion error: %v", err))
